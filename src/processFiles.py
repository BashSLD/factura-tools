...
def process_files_logic(carpeta, sucursal, progress_callback=None):
    if carpeta is None:
        raise ValueError("Selecciona una carpeta antes de continuar.")

    archivos_xml = [f for f in os.listdir(carpeta) if f.lower().endswith('.xml')]
    total_archivos = len(archivos_xml)
    if not archivos_xml:
         raise FileNotFoundError("No se encontraron archivos XML en la carpeta.")
    ...
    if data:
        ...
        if sheet:
            ...
            # Retornamos la ruta del excel para que la UI decida qué hacer (abrirlo o solo notificar)
            return ruta_excel
        else:
             raise Exception("Error al acceder al archivo Excel generado.")
    else:
        raise FileNotFoundError("No se encontraron archivos XML válidos para procesar.")